...
* `additional_properties`: For event types that declare compatibility
...
[#210]
== {SHOULD} Avoid `additional_properties` in event type definitions

Event type schema should avoid using `additional_properties`
...
to evolve safely over time *must not* declare an `additional_properties`
...
the absence of an `additional_properties` field as though the event type
...
are prevented by not using `additional_properties`.

See also rule <<111>> in the <<compatibility>> section for further guidelines 
on the use of `additional_properties`.
